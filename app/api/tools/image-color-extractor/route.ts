...
import { extractImageColors, ImageColorExtractorInput } from '@/lib/tools/image-color-extractor'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (input.colorCount !== undefined) {
      if (!Number.isInteger(input.colorCount) || input.colorCount < 1 || input.colorCount > 10) {
        return validationErrorResponse('colorCount must be an integer between 1 and 10')
      }
    }
    ...
    const result = extractImageColors(input)

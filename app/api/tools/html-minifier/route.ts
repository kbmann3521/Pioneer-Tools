...
export async function POST(request: NextRequest) {
  try {
    ...
    const bodyValidation = validateToolRequestBody(body, ['html'])
    if (!bodyValidation.valid) {
      return bodyValidation.error
    }

    const { html } = bodyValidation.data!
    ...
    return internalErrorResponse('Failed to process HTML')
  }
}
...
export async function GET() {
  return successResponse({
    name: 'HTML Minifier',
    description: 'Minify and optimize HTML code',
    endpoint: '/api/tools/html-minifier',
    method: 'POST',
    parameters: {
      html: { type: 'string', required: true, description: 'HTML code to minify' },
    },
  })
}

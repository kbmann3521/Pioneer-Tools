...
export async function POST(request: NextRequest) {
  try {
    ...
    const input = body as OGGeneratorInput

    // Validate required fields
    if (!input.title || !input.description || !input.url || !input.image) {
      return validationErrorResponse('Missing required fields: title, description, url, image')
    }
